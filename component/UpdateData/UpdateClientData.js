...
import BodyRow6 from "../HomePage/BodyRow6";
...
const UpdateClientData = (props) => {
  ...
  const onFinish = (values) => {
    updateDataToClient(values);
  };

  const updateDataToClient = (values) => {
    const api = {
      method: "POST",
      headers: {
      ...
      body: JSON.stringify({
        query: `mutation{
          update_Clients(where: {Email: {_eq:${props.clientRecord.Email}}}, _set:{Name:${values.name},Email:${values.email},Status:${values.status}}){
            returning{
              Name
            }
          }
      ...
      >
        ...
        >
          ...
          <Form.Item {...buttonItemLayout}>
            <Button type="submit" htmlType="submit" className="modalButton" onClick={()=>BodyRow6 } >
              Update
            </Button>
          </Form.Item>
        </Form>
